...
pub fn solve(input: &str, n: usize) -> (u64, u64) {
    ...
    let (shortest_dist, _, _) = all_pairs_dist_ind.select_nth_unstable(n);
    shortest_dist.sort();

    let mut indexes_to_circuits = HashMap::new();
    let mut next_avaliable_circuit_index = 0;
    for (_, i1, i2) in shortest_dist {
        let i1_circuit = indexes_to_circuits.get(&i1);
        let i2_circuit = indexes_to_circuits.get(&i2);
        match (i1_circuit, i2_circuit) {
            (None, None) => {
                indexes_to_circuits.insert(i1, next_avaliable_circuit_index);
                indexes_to_circuits.insert(i2, next_avaliable_circuit_index);
                next_avaliable_circuit_index += 1;
            },
    ...
    let ans1 = circuits_total_3_top.iter().map(|x| **x).product::<u64>();
    ...
    for (_, i1, i2) in all_pairs_dist_ind.iter() {
        let i1_circuit = indexes_to_circuits.get(&i1);
        let i2_circuit = indexes_to_circuits.get(&i2);
        match (i1_circuit, i2_circuit) {
            (None, None) => {
        ...
        dbg!(circuits_total, indexes_to_circuits.len(), coordinates.len());
        if circuits_total == 1 && indexes_to_circuits.len() == coordinates.len() {
            println!("{:?} {:?}", coordinates[*i1], coordinates[*i2]);
